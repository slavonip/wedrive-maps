...
# Run as the unprivileged `diagnostic` account with passwordless sudo, not as root: root's
# password is expired on this image and PAM refuses a non-interactive session even after the key
# authenticates. Every command that needs privilege says so.
S=sudo

section() { printf '\n========== %s ==========\n' "$1"; }

section "stages — the last PASS is where it got to"
$S cat /var/log/wedrive-stages 2>/dev/null || echo "no stage file: the bootstrap script never ran"

section "bootstrap script output"
$S tail -100 /var/log/wedrive-bootstrap.log 2>/dev/null || echo "no bootstrap log"

section "cloud-init status"
$S cloud-init status --long 2>&1

section "cloud-init units"
$S systemctl status cloud-init cloud-init-local cloud-config cloud-final --no-pager 2>&1 | head -60

section "cloud-init output, last 80 lines"
$S tail -80 /var/log/cloud-init-output.log 2>&1

section "cloud-init log, errors only"
$S grep -iE "error|fail|traceback|warn" /var/log/cloud-init.log 2>/dev/null | tail -40

section "github-runner unit"
$S systemctl status github-runner --no-pager 2>&1 | head -40

section "github-runner journal"
$S journalctl -u github-runner --no-pager 2>&1 | tail -80

section "the unit as systemd sees it"
$S systemctl cat github-runner --no-pager 2>&1 | sed 's/\(--jitconfig \)[A-Za-z0-9+/=]\{16\}[A-Za-z0-9+/=]*/\1<REDACTED>/'

section "/opt/runner"
$S ls -la /opt/runner 2>&1 | head -30
echo "run.sh present: $($S test -x /opt/runner/run.sh && echo yes || echo NO)"
...
section "account state — the evidence for or against the `su - runner` theory"
$S passwd -S root 2>&1
$S passwd -S runner 2>&1
$S chage -l runner 2>&1 | head -6

section "apt and unattended-upgrades — the lock that breaks boot-time installs"
systemctl is-active unattended-upgrades 2>&1
$S fuser -v /var/lib/dpkg/lock-frontend 2>&1 | head -5
$S tail -20 /var/log/unattended-upgrades/unattended-upgrades.log 2>/dev/null
